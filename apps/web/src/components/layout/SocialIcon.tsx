...
/**
 * Resolves a social link's `icon` field to a visual: a known brand preset,
 * an uploaded custom image, or a plain-text fallback when neither matches.
 */
export default function SocialIcon({ icon, platform }: { icon: string; platform: string }) {
  const preset = PRESET_ICONS[icon.toLowerCase().trim()]
  if (preset) {
    return <>{preset}</>
  }

  if (isImagePath(icon)) {
    return <img src={resolveImageUrl(icon)!} alt={platform} className="w-5 h-5 object-contain" />
  }
